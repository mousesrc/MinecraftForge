...
public class EnumHelper {
    ...
    //MCPCRevive
    //Readd some methods to patch bukkit
    //including -> EnvironmentType, Biomes, EntityType, Material, Inventroy
    //start
    public static InventoryType addBukkitInventory(Object TE)
    {
        ...
        {
            throw new IllegalArgumentException("Assigned TE " + TEClass + " didn't implement IInventory");
        }
    }
    public static World.Environment addBukkitEnvironment(DimensionType type)
    {
    ...
    {
        String name = generateName(enchantment);
        Function<org.bukkit.Material,Boolean> TheFunction = new Function<org.bukkit.Material, Boolean>() {
            @Override
            public Boolean apply(org.bukkit.Material material) {
    ...
    public static String generateName(net.minecraft.enchantment.Enchantment target) {
        ...
        if (clz.getName().startsWith("net.minecraft")) {
            ...
            candidate = "UNKNOWN_ENCHANT_" + target.getRegistryName().toString();
            return candidate;
        }
    ...
    private static String addSuffix(String enchName) {
        if (org.bukkit.enchantments.Enchantment.getByName(enchName) == null) {
            return enchName;
        }

        int suffix = 2;
        while (org.bukkit.enchantments.Enchantment.getByName(enchName + "_" + suffix) != null) {
            suffix++;
        }
