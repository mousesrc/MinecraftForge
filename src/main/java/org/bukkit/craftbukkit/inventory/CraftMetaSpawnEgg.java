...
public class CraftMetaSpawnEgg extends CraftMetaItem implements SpawnEggMeta {
    ...
    CraftMetaSpawnEgg(NBTTagCompound tag) {
        ...
        if (tag.hasKey(ENTITY_TAG.NBT)) {
            ...
            if (entityTag.hasKey(ENTITY_ID.NBT)) {
                this.spawnedType = EntityType.fromName(new ResourceLocation(entityTag.getString(ENTITY_ID.NBT)).getResourcePath()); // PAIL: rename
            }
        }
    }
    ...
    void deserializeInternal(NBTTagCompound tag) {
        ...
        if (tag.hasKey(ENTITY_TAG.NBT)) {
            entityTag = tag.getCompoundTag(ENTITY_TAG.NBT);
            ((CraftServer)Bukkit.getServer()).getServer().getDataFixer().process(FixTypes.ENTITY, entityTag); // PAIL: convert
    ...
    void applyToItem(NBTTagCompound tag) {
        ...
        if (hasSpawnedType()) {
            ...
            entityTag.setString(ENTITY_ID.NBT, new ResourceLocation(domain_and_name[0],spawnedType.getName()).toString().toLowerCase()); //MCPC revive , fix spawn egg meta problem
        }
