...
public class CraftContainer extends Container {
    ...
    private void setupBeacon(IInventory top, IInventory bottom) {
        ...
        this.addSlotToContainer(new Slot(top, 0, 136, 110));

        int i;

        for (i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlotToContainer(new Slot(bottom, j + i * 9 + 9, 36 + j * 18, 137 + i * 18));
            }
        }

        for (i = 0; i < 9; ++i) {
            this.addSlotToContainer(new Slot(bottom, i, 36 + i * 18, 195));
        }
    ...
    private void setupShulkerBox(IInventory top, IInventory bottom) {
        ...
        for (i = 0; i < 3; ++i) {
            for (j = 0; j < 9; ++j) {
                this.addSlotToContainer((Slot) (new SlotShulkerBox(top, j + i * 9, 8 + j * 18, 18 + i * 18)));
            }
        }
